...
public class fcliente {
    ...
    public DefaultTableModel mostrar(String buscar) {
        ...
        String[] titulos = {"ID", "Nombre", "Apellidos", "Nacionalidad", "Tipo. Cli", "Númeo Documento", "Dirección", "Teléfono", "Email", "Código","dateadd"};
        ...
        sSQL = "select p.idpersona,p.nombre,p.apaterno,p.amaterno,p.tipo_documento,p.num_documento,"
                + "p.direccion,p.telefono,p.email,p.dateadd,c.codigo_cliente from persona p inner join cliente c "
        ...
        try {
            ...
            while (rs.next()) {
                ...
                registro [10]=rs.getString("dateadd");
    ...
    public boolean insertar(vcliente dts) {
        sSQL = "insert into persona (nombre,apaterno,amaterno,tipo_documento,num_documento,direccion,telefono,email,dateadd)"
                + "values (?,?,?,?,?,?,?,?,?)";
    ...
    public boolean editar(vcliente dts) {
        sSQL = "update persona set nombre=?,apaterno=?,amaterno=?,tipo_documento=?,num_documento=?,"
                + " direccion=?,telefono=?,email=?,dateadd=? where idpersona=?";
